...
Say I am tracking the height of some rice plants over time.
...
```{r}
rice_growth <- read_csv("rice_growth_data.csv")
head(rice_growth)
```
...
since it is both the EU for genotype, and it is the entity of repeated measures. 
...
```{r}
model_rice <- lmer(Height_mm ~ age.f * genotype + (1|flat) + (1|pot), data = rice_growth)
```
...
That means the variance is even across groups. 
