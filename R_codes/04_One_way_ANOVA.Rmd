...
We will compare bill length across the three species\
and find out who (if anyone) has the longest bill.
...
2)  Independence: this means the measurement of one observation does not affect the other\
...
3)  Normality: the residues (errors) are normally distributed\
...
4.  The last assumption is homoscedasticity: equal variance across groups.\
...
For example, in row 1, we have t = -23, if t were to be 0, the probability of finding t \< -23 is \<0.0001.\
Very small.\
...
If n \> 10 but \< 100, it's a good idea to show the boxplot.\
