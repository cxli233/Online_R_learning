...
I encourage you to read through this lesson to familiarize yourself with the R synta
x.

As a resource, here's the link to the [base R cheat sheet](https://rstudio.com/wp-content/uploads/2016/10/r-cheat-sheet-3.pdf)
...
RStudio is a user interface, which is a lot more user-friendly than plain R itsel
f.
...
1)  Script Area:\
    ...
    Once you are done with a script file, you can also save it on your compute
    r.
...
2)  The Console Area:\
    ...
    This is why we use script files (which are save-able) instead of directly typing in the con
    sole.
...
4)  The Miscellaneous Area:\
    ...
    If you need help on a particular command, you can typ
    e `?` followed by the command in the console.\
    For example, if you need help on the `sum()` command, ty
    pe `?sum()` in the console.\
    R will pull out the documents regarding the `sum()` command,\
    and it will show up at the Help tab at the lower
    right.
...
A difference between R and many other coding languages (e.g., C) is that\
in R you learn more about how to use other packages, and less about coding stuff yourself.\
This is very advantageous for those who do not have a traditional computer science backgro
und.
...
You will type `install.packages("dplyr")` at the consol
e.
...
We will talk about how to load an installed package late
r.

## The R Markdown (.Rmd) file

R Markdown is an enhanced R script file.
You are reading a R markdown file right now.\
...
To run the whole chunk, do Ctrl+Shift+Enter on PC or Cmd+Shift+Enter on
Mac.
...
To make a new section header, you can type `#` followed by a space, and then\
any text at the first position of any line in the plain text area.\
In addition, `##` will be a sub-header, and `###` will be a sub-sub-header, and so o
n.
...
If you click on any section titles, RStudio will jump to that sectio
n.
...
If you save them as the same name, the later item will just overwrite the previou
s.
...
Forgetting to use quotes is a very common source of bug
s.
...
Say I want the 2nd member in vector `a`, the code will b
e
...
Today we will just have a brief introductio
n.
...
In this case, the column names are name and co
unt.
...
`%>%` is a good trick.\
Missing parentheses is another common source of bugs, and `%>%` really reduces tha
t.
...
The shortcut is Ctrl+Shift+C on a PC and Cmd+Shift+C on a Ma
c.
...
\
3.
Select the 1st and 3rd rows and the 1st, 2nd and 4th columns, and save it as an item.

4\.
Take the square root for each member of my_mat2, then take log2(), and lastly find the maximum value.
Use the pipe syntax.\
