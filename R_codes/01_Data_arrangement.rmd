...
##Data frames 
This table is called a data frame in the jargons of R. A data frame is simply a data table. It has rows and columns. 
An important aspect of data frame is that rows of the same column must be the same class of variable. 
For example, in column A, all values in column A are numeric. In column "treatment", all values in that column are characters. 
...
1: the "key" column (sometimes called the label column) is "observation": it houses which original columns the data came from (A - F).
...
We have 4 variables: group, treatment (4 treatments), observations (A up to F), and count of DAPI bodies (numeric). 
...
In this experiment, you are looking at the expression of three genes (gene1 - gene3) in three samples (sample1 - sample3). 
...
how to subset a data frame, how to creat new columns in a data frame and summarise a data frame.
...
Using our tidy DAPI data now, say we only want the columns count and treatment and leave out the rest of the columns. 
...
nrow(DAPI_tidy_3)
```
Now the data frame went from 456 rows to 205 rows. So the students actually made a total of 205 observations. 
...
head(DAPI_tidy_room)
```
case_when() is an extremely useful command within mutate(). The syntax is case_when(condition ~ outcome). 
...
```
1) group_by() selects which columns you want to summarise from. In this case you just want each treatment to be a group. 
...
Now you just get 4 rows and 4 columns, each row is a treatment. The summary statistics are what people want to read in publications. However, you would still want to release raw data as a supplemental file. 
...
```
1) This time you need to select group and treatment in group_by(). Because you want the mean for each group and each treatment. 
...
In data analyses, the data might not be all in one file. It is common that the data of interest were recorded in multiple spreadsheets. 
...
These tables are not in the tidy format! The year values are spread out across many columns, so you need to gather them first. 
```{r}
