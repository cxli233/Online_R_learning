...
Let's make the most basic plot fir
st
...
(`alpha = 1` is 0% transparent and default.)\
...
To help you understand the difference between color and fill, let's compare the following two plot
s.
...
instead of `scale_color_manual()`, because the variable `supp` is now reflected by fill, not colo
r.
...
How do I fix tha
t?\
You can add a `seed` argument within `position_jitter()`, that will make it reproduci
ble
...
It's easy to fi
x. We will use `scale_color_manual()` to specify colors for the lines to
o.
...
linerange is errorbar without the horizontal b
ars
...
But feel free to use errorbar or linerange if you prefer thos
e
...
The white background and black lines really put the data forward and emphasize the
data
...
and `c(1, 0)` means the lower right corner of the graph are
a
...
faceting allows you and readers to focus on the true comparison of intere
st
...
`facet_grid(row ~ column)` We are using supp as columns, and only 1 row, so `facet_grid(. ~ supp)`. The
`.` before the `~` is just a space holder
.
...
So what we really need is to put supp on x axis inste
ad
...
theme(axis.line = element_line(size = 1.2)) +\
theme(text = element_text(size = 12, color = "black", face = "bold")) +\
theme(axis.text = element_text(size = 12, color = "black", fa
ce = "bold"))
