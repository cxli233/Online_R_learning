...
I expect 1/4 AA, 1/2 Aa and 1/4aa. 
...
We have 3 categories, AA, Aa and aa, so df = 3 - 1 = 1. 

```{r}
#don't worry about this chunck. It simulates a Chi-squared distribution if df = 2 and n = 10^4.
...
The null hypothesis of Chi-squared is 0. A Chi-squared value of 0 would mean observed data fits expected proportions perfectly.
The p value is computated as the right hand side area under the graph when x > Chis-quared, which means
...
#Contigency tables - Chi-squared test and Fisher Exact test
Now let's switch gears to talk about a related analysis, which is contingency tables, 
which is an analysis to study classfications and association. 

Say in a hypothetical transcriptome study, 30000 genes were detected as expressed. 
Among them, 18000 genes were upregulated by UV light treatment, whereas 12000 genes were downregulated or unchanged. 
Meanwhile, 24000 genes were upregulated by heat stress, where as 6000 were downregulated or unchanged. 
...
So, yes. But what is the statisitical way to quantify this? 

Here is where contingency tables are useful: 
           | UV-up | not UV-up| total | 
...
In this case, you feed the contigency table (without the "total" rows and columns) into the x argument of the Chisq.test() function
...
Fisher Exact test also returns a number called the odds ratio (OR) for 2x2 contingency tables.
In a contingency table like this:
  | a | b |
  | c | d |
OR = (a/c)/(b/d). So in case it will be (15000/3000) / (9000/3000) = 1.67, which means
it's 1.67 times more likely that a UV-upregulated genes is also heat-upregulated. 
Don't worry about why the Fisher estimate is not exactly the same as the arithmatic OR, 
...
The farther away OR is from 1, the small the p value, and less likely the null hypothesis is true. 


Let's do another example for Fisher exact test.
In this hypothetical example, a person claims they can tell the difference between a can of coca cola and pepsi. 
Here is the contigency table:
              | acual coke | actual pepsi | total
claimed coke  |  8         |    2         |  10
claimed pepsi |  3         |    7         |  10
total         |  11        |    9         |  20 

The columns are whether the drink was actually a cola or a pepsi.
The rows are whether the person claims it was a colo or a pepsi. 
...
The sunflower plant track the sun during the day from east to west, and re-orient to the east during the morning. 
...
among which 2000 of them have the annotation of growth and cell elongation related. 

And you have a list of 100 upregulated genes from 12000 expressed genes in this morning-east stem sample, 
among which 40 of them have the annotation of growth and cell elongation related.

There are actually two tests to approach this problem.
The first one is using a contigency table. SO let's set up the contingency table. 
