...
Inside the pairwise.wilcox.test(), 
you will use ```DAPI_tidy$count``` to call the count column from the DAPI_tidy data frame.    
...
As you can see the P values are all very low,
suggesting the median counts of DAPI bodies for all treatments are different from each other.     
...
For example, if p = 0.5, then the odds are 1 : 1. If p = 0.1, then the odds are 1 : 9.   
...
When you calculted the means in the transformed scale then de-transform,   
...
let't just use one of the cultivars - "Alias".    
