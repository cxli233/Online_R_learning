...
It is the -log10 value of H+ cencentration in M, or mol/L. Say pure water has [H+] = 10^-7 M, thus pH = 7. 
...
This is not a tidy data frame. Let's make it a tidy data frame by gathering the observations,
...
```
Looking at the graph, you may have noticed a problem. The spread of each treatment differs widely. 
...
We can actually take a look at the summary statistics and have a direcly look at the variances. 
```{r}
...
But to illustrate the principles, let's set up a linear model in the original scale anyway and see how things will go wrong. 
```{r}
...
```
I think this looks pretty bad. The errors at either extremes are VERY far from normally distrubuted. 

Equal variances?
```{r}
plot(model_DAPI, which = c(1, 3))
```
These also look pretty bad. Again we can clearly see that treatments with larger means have higher variances. 
...
It might have helped a little bit, but the errors were still not normally distrubuted. 
...
3) Do non-parametric tests, such as the wilcoxon test. 
4) Try generalized linear models. These are a lot more complicated, and we won't cover them today. 


#Wilcoxon tests
The term non-parametric tests refer to statistical tests that do not rely on pre-existing distrubutions. 
...
Wilcoxon test, also called the Mann–Whitney U test, is a very commonly used non-parametric test.  
...
Fortunately, R has a build-in pariwise.wilcox.test() function.
...
The "$" symbol calls a particular column from a data frame. You will do the same for treatment as well. 
```{r}
pairwise.wilcox.test(x = DAPI_tidy$count, g = DAPI_tidy$treatment)
```
Here are the results. We have 6 comparisons, and the P values are shown in this table. 
...
this is indicated by the message "P value adjustment method: holm". So we don't need to worry about multiple comparisons ourselves. 


#logit transformation
Another very useful data transformation is the logit transformation. Let's start with an example. 
```{r}
...
C_elegans_lethality %>% 
  ...
  theme(axis.text = element_text(size = 12, color = "black", face = "bold"))
```
Again Looking at the graph, we may have noticed a problem. The spread of each treatment differs widely,
but in a way that is different from the DAPI count data. 
In this case, the smaller and larger treatments have smaller spread, while treatments in the mid-range has larger spreads. 
...
Again the variances differ for more than a 2-fold change. So it will not meet the assumptions for linear model. 
...
This is where the logit transformation comes in handy. It is defined as: 
...
For example, if p = 0.5, then the odds are 1 : 1. If p = 0.1, then the odds are 1 : 9. If p = 0.01, then the odds are 1 : 99.  

The reverse operation for logit() is logistic(). It is defined as:
...
When you need to use logit and logstics in your future codes, you can come back here and copy and paste the above chunck.
...
When you logit transform a variable, it spreads out the values at either ends, but squeezes the values in the mid-range.
...
In this lethality experiment, the data are already proprotions, so we don't need to scale down here. 
...
which are all values that are not 0 or 1, set the new proportions to be the original proprotions, no change.  
...
```
I think the normal qqplot has improved a lot. So the logit transformation was definately helping. 
The variances across treatments are also a lot more even. So we should proceed with the logit transformed data for linear model. 

##How to interpret transformed data? 
When you decided to proceed with data transformation, it's a good idea to run the same analyses on both transformed and original data. 
...
```
You can see that in the original scale, the means of rec-8 and syp-2 are detected as not statistically different (P = 0.21 > 0.05). 
But in the logit transformed scale, they are statiscally different (P = 0.015). 
Again, the results for logit transformed data should be more valid, as the data meet the assumptions better.

Overall the results are pretty similar though. So if the differences are large enough, it practically doesn't change your conclusions. 
...
However, we have one more thing to do here. Look at the emmean column. These are the estimated means.
They are reported in the logit scale, not the original scale. The same for all other values. 
To make sense of them, we need to de-transform them back to proportions using the logistic() funciton. 
...
lethal_detransform <- cld(estimate_lethal_logit$emmeans, Letters = letters) %>% 
  ...
  select(treatment, mean, estimated_mean, lower.CL_detrans, upper.CL_detrans, .group, n)
...
When you calculted the means in the transformed scale then de-transform, the estimated means are different from the arithmetic means. 
...
(FYI: if I remember the biology correctly, syp-2 RNAi has a catastrophic effect on meiosis I and should lead to ~100% dead embryos,
whereas as the empty vector control L4440 should have ~0% dead embryos. Comparing the means and de-transformed estimated means, 
...
At the end of the experiment, they counted the number of shoots from each tissue (Buds_Shoots column in the table).  
There were other factors in this experiment, but let's ignore them for now. 
They did this for a total of 19 daylily cultivars. For the sake of this exercise, let't just use one of the cultivars - "Alias".
...
###Calculate the summary statitics using group_by() and summarise() for Buds_Shoots
```{r}
