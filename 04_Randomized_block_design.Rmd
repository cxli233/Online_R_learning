...
Last time we went over how to perform one-way ANOVA to compare means of multiple treatments.
...
And say this experiment happened in the northen hemisphere. 
...
factor strans for is it a blocking factor? Is it a treatment? Is it an observation?
...
1) The numerically greatest mean is due to pure chance. And if you were to perform the experiment again, 
the mean of H may be lower next time. 
...
Again, when you interpret P values and the letter grouping, be thoughtful and careful on what they could mean, 
...
```{r}
chicken_experiment <- read_csv("betacarotenoid.csv")
...
Pull out the cld output. What does it tell you? 
